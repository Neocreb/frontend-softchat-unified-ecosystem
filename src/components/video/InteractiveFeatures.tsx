...
interface SmartSuggestion {
  id: string;
  type: 'hashtag' | 'mention' | 'music' | 'effect' | 'caption';
  content: string;
  confidence: number;
  relevance: number;
}
...
const InteractiveFeatures: React.FC<InteractiveFeaturesProps> = ({
  ...
  const generateAutoCaptions = async () => {
    ...
    try {
      ...
      const newCaptions: CaptionTrack[] = [
        {
          id: '1',
          language: 'en',
          content: 'This is an auto-generated caption for the video content.',
          confidence: 0.92,
        ...
        },
        {
          id: '2',
          language: 'es',
          content: 'Esta es una leyenda generada automáticamente para el contenido del video.',
          confidence: 0.88,
          isAutoGenerated: true,
          editable: true,
        },
      ...
      toast({
        ...
        description: "AI has generated captions in multiple languages.",
      });
    } catch (error) {
      toast({
        title: "Caption Generation Failed",
        description: "Unable to generate captions. Please try again.",
        variant: "destructive",
      });
    } finally {
  ...
  const generateSmartSuggestions = async () => {
    const suggestions: SmartSuggestion[] = [
      { id: '1', type: 'hashtag', content: '#viral', confidence: 0.95, relevance: 0.92 },
      { id: '2', type: 'hashtag', content: '#trending', confidence: 0.88, relevance: 0.85 },
      { id: '3', type: 'hashtag', content: '#fyp', confidence: 0.92, relevance: 0.89 },
      { id: '4', type: 'mention', content: '@influencer', confidence: 0.76, relevance: 0.82 },
      { id: '5', type: 'caption', content: 'Check out this amazing moment!', confidence: 0.84, relevance: 0.87 },
    ];
  ...
  // Enhanced share function with more platforms
  const handleEnhancedShare = (platform: string) => {
    const videoUrl = `https://softchat.com/video/${videoId}`;
    const videoTitle = "Check out this amazing video on Softchat!";
    ...
    switch (platform) {
      ...
      case "download":
        ...
        link.href = videoUrl;
        link.download = `softchat-video-${videoId}.mp4`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        toast({
          title: "Download Started",
          description: "Video download will begin shortly",
        });
        break;
      case "twitter":
      ...
      case "whatsapp":
        ...
        break;
      case "email":
        window.open(`mailto:?subject=${encodeURIComponent(videoTitle)}&body=${encodeURIComponent(`Check out this video: ${videoUrl}`)}`);
        break;
      default:
        toast({
          title: "Share Feature",
          description: `Sharing to ${platform} - Feature coming soon!`,
        });
    }
